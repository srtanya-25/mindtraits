...
def _rule_based_result(scores: dict) -> dict:
    """Rule-based prediction + proportional trait contribution (no ML libs needed)."""
    total = sum(scores.values()) or 1
    return {
        "predicted_type": _rule_based_label(scores),
        "shap_explanation": {
...
def predict_personality(scores: dict) -> dict:
    ...
    Returns:
        {'predicted_type': str, 'shap_explanation': dict}  # feature -> impact_percent
    """
    # No trained model -> skip the heavy ML imports (shap/numba/xgboost) entirely.
    # Importing them on a memory-constrained host can OOM-kill the worker.
    if not os.path.exists(MODEL_PATH):
        return _rule_based_result(scores)
    ...
    try:
        ...
        # Any failure in the ML path -> safe rule-based fallback
        return _rule_based_result(scores)
